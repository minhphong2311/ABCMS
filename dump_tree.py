...
async def main():
    async with async_playwright() as p:
        ...
        res = await page.evaluate('''async () => {
            ...
            return await s.getTreeList('test-phong');
        }''')
        
        with open('tree_dump.txt', 'w', encoding='utf-8') as f:
            for item in res.get('items', []):
                depth = item.get('depth', 1) or 1
                name = item.get('menuNm', '')
                parent = item.get('parentMenuCd')
                cd = item.get('menuCd')
                f.write(f"{'  ' * (depth-1)}- {name} (cd: {cd}, parent: {parent}, depth: {depth})\n")
                
        print('Dumped tree to tree_dump.txt')
        await browser.close()
